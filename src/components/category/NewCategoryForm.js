...
const NewCategoryForm = ({ initialValues, handleClose, onDataSubmit }) => {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (!initialValues) {
          ...
          setLoading(false);
          onDataSubmit('Category created successfully');
        } else {
          const docRef = doc(firestore, 'Categories', initialValues?.id);
          const newCategory = {
            categoryName: values.name,
          };
          await updateDoc(docRef, newCategory);
          setLoading(false);
          onDataSubmit('Category updated successfully');
        }
      } catch (err) {
      ...
      <form onSubmit={formik.handleSubmit} id="profileForm">
        ...
        >
          <Typography variant="h6" gutterBottom>
            {initialValues ? 'Update Category' : 'New Category'}
          </Typography>
